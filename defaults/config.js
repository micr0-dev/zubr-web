...
module.exports = {
    ...
    // ZUBR-WEB: defaults must match homeServer for lockNetwork validation
    // Network settings are fetched dynamically from zubr-server
    defaults: {
        name: "Home Server",
        host: "127.0.0.1",
        port: 6667,
    ...
    //
    // ZUBR-WEB: Set to false to allow federation (adding remote servers later)
    lockNetwork: false,

    // ## ZUBR-WEB Configuration

    // ### `zubrServer`
    //
    // Configuration for zubr-server API integration.
    //
    // The available keys for the `zubrServer` object are:
    //
    // - `enabled`: When set to `true`, authentication is handled by zubr-server API.
    //   This value is set to `true` by default.
    // - `url`: The base URL of the zubr-server API. Defaults to `http://localhost:3000`.
    //
    zubrServer: {
        enabled: true,
    ...
    // ### `homeServer`
    //
    // Configuration for the home IRC server that all users auto-connect to.
    //
    // The available keys for the `homeServer` object are:
    //
    // - `enabled`: When set to `true`, users automatically connect to a home IRC server.
    //   This value is set to `true` by default.
    // - `name`: Display name for the home server. Defaults to "Home Server".
    // - `host`: IRC server hostname/IP. Defaults to `127.0.0.1`.
    // - `port`: IRC server port. Defaults to `6667`.
    // - `tls`: Enable TLS connections. Defaults to `false`.
    // - `rejectUnauthorized`: Verify server certificate. Defaults to `true`.
    // - `channels`: Array of channel names to auto-join. Defaults to `["#general"]`.
    //
    homeServer: {
        enabled: true,
        name: "Home Server",
